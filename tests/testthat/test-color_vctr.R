test_that("New color vectors by default have no styling", {
  ...
  expect_equal(length(new_vect), 1)
  expect_equal(vctrs::field(new_vect,".text_color"), NA)
  expect_equal(vctrs::field(new_vect,".background"), NA)
  expect_equal(vctrs::field(new_vect,".style"), NA)
})
...
test_that("Text_color provided must be either length 1 or length of input vector",{
  ...
  expect_error(
    color_vctr(c(1,2,3,4), text_color = c("blue","yellow","magenta"))
  )
